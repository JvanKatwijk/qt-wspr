 ...
 *    along with noaa eceiver if not, write to the Free Software
...
// 	Note, tables only for band III (120 - 250 MHz)
static
int     RSP1_Table [5] = {4, 0, 24, 19, 43};

static
int     RSP1A_Table [11] = {10, 0, 6, 12, 18, 20, 26, 32, 38, 57, 62};

static
int     RSP2_Table [10] = {9, 0, 10, 15, 21, 24, 34, 39, 45, 64};

static
int     get_lnaGRdB (int hwVersion, int lnaState) {
        switch (hwVersion) {
           case 1:
	      if (lnaState <= RSP1_Table [0])
                 return RSP1_Table [lnaState + 1];

           case 2:
	      if (lnaState <= RSP2_Table [0])
                 return RSP2_Table [lnaState + 1];

           default:
	      if (lnaState <= RSP1A_Table [0])
                 return RSP1A_Table [lnaState + 1];
        }
	return 1;
}

int	lnaStates (int hwVersion) {
	switch (hwVersion) {
	   case 1:	
	      return 5;
	   case 2:
	...
	sdrplayHandler_v2::sdrplayHandler_v2  (RadioInterface *mr,
...
sdrplaySelect	*sdrplaySelector;
	...
	this	-> outputRate	= 192000;
	this	-> inputRate	= 12 * outputRate;
...
//	Ok, make explicit it is in the 64 bits section
	...
	filter_1	= new decimator (2 * 4 + 1, -outputRate / 2,
	                                           + outputRate / 2,
	                                           inputRate, 3);
	filter_2	= new decimator (2 * 8 + 1, -outputRate / 2,
	                                           + outputRate / 2,
	                                           inputRate / 3, 4);
	 
	api_version	-> display (ver);
	lastFrequency	= Khz (137000);
//
...
void    sdrplayHandler_v2::newdataAvailable (int n) { 
        dataAvailable (n);
}
...
int     lnaState        = lnaGainSetting        -> value ();
	...
	if (MHz (120) > newFrequency)
	   return;
	if (MHz (250) < newFrequency)
	   return;
	if (!running. load ()) {
...
static int teller = 0;
	...
	for (i = 0; i < (int)numSamples; i ++) {
	   ...
	   if (p -> filter_1 -> Pass (temp, &temp)) 
	      if (p -> filter_2 -> Pass (temp, &localBuf [cnt]))
	         if (localBuf [cnt] == localBuf [cnt])
	            cnt ++;
	}

	p -> _I_Buffer -> putDataIntoBuffer (localBuf, cnt);
	if (p -> _I_Buffer -> GetRingBufferReadAvailable () >= 48000)
	   p -> newdataAvailable (48000);
	(void)	firstSampleNum;
	(void)	grChanged;
	(void)	rfChanged;
	(void)	fsChanged;
